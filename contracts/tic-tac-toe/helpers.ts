 ...
 *
 * Usage: npx @cosmjs/cli@^0.23 --init https://raw.githubusercontent.com/CosmWasm/cosmwasm-examples/main/TicTacToe/helpers.ts
 *
 * Create a client:
...
interface TicTacToeContract {
  ...
  instantiate: (codeId: number, initMsg: InitMsg, label: string) => Promise<TicTacToeInstance>

  use: (contractAddress: string) => TicTacToeInstance
}

const TicTacToe = (client: SigningCosmWasmClient): TicTacToeContract => {
  const use = (contractAddress: string): TicTacToeInstance => {
  ...
  const upload = async (): Promise<number> => {
    const meta = {
      source: "https://github.com/CosmWasm/cosmwasm-examples/tree/TicTacToe-0.7.0/TicTacToe",
      builder: "cosmwasm/rust-optimizer:0.10.4"
    };
    const sourceUrl = "https://github.com/CosmWasm/cosmwasm-examples/releases/download/TicTacToe-0.7.0/contract.wasm";
    const wasm = await downloadWasm(sourceUrl);
    const result = await client.upload(wasm, meta);
    return result.codeId;
  }

  const instantiate = async (codeId: number, initMsg: Record<string, unknown>, label: string): Promise<TicTacToeInstance> => {
...
// const factory = TicTacToe(client)
//
